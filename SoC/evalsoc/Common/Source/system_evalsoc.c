...
uint32_t core_exception_handler_s(unsigned long scause, unsigned long sp)
{
    ...
    void SystemBannerPrint(void)
    {
        ...
        NSDK_DEBUG("PAICORE SDK Build Time: %s, %s\r\n", __DATE__, __TIME__);
...
// TODO: If you don't need to support SMP, you can directly remove code in it
    ...
    if (ICachePresent())
    { // Check whether icache real present or not
#if defined(RUNMODE_ECC_EN)
#if RUNMODE_ECC_EN == 0
    ...
    if ((mcfginfo & (0x1 << 11)) && SMP_CTRLREG(__SMPCC_BASEADDR, 0x4) & 0x1)
    { // L2 Cache present
#if RUNMODE_L2_EN == 1
