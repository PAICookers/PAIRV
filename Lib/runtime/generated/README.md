...
│       │       ├── name, shape, bit_width, tick
│       │       └── entries[]: InputEntry
│       ├── output_mappings
│       │   ├── target_lcn
│       │   └── items[]: OutputTensorMapping
│       │       ├── name, shape, kind, bit_width, tick
│       │       └── entries[]: OutputEntry
│       └── core_ticks[]: CoreTick
├── config_frames
│   ├── words[]
...
| `TickParams` | `tick_start`       | PAICORE 节点开始工作的硬件 tick。         |
...
|                     | `output_mappings`  | PAICORE DATA frame 到输出 tensor 的映射集合。       |
...
|                          | `kind`         | 输出是 DATA 激活值还是 VOLTAGE 膜电位。       |
|                          | `bit_width`    | 输出值位宽。                                  |
|                          | `tick`         | 输出对应的 tick 参数。                        |
...
|                        | `VOLTAGE`    | 膜电位输出，语义为 int32；当前 runtime 尚未执行该解码路径。 |
...
`ExecutionPlan` 是 target-local 的有序 runtime 调度计划。
...
`words[]` 长度必须为偶数。PAIRV 每次读取完整 high/low pair，并在 session
运行 sample 前一次性加载全局配置。
