...
static void rv_debug_default_sink(rv_debug_level_t level, const char *title,
                                  const char *function_name,
                                  const char *message, void *user_data)
{
    (void)user_data;
    printf("[%s][%s] %s: %s\r\n", title ? title : "debug",
           rv_debug_level_name(level), function_name ? function_name : "-",
           message ? message : "");
}

void rv_debug_set_level(rv_debug_level_t level) { g_rv_debug_level = level; }
...
static void rv_debug_vlogf(rv_debug_level_t level, const char *title,
                           const char *function_name, const char *fmt,
                           va_list ap)
{
    if ((level <= RV_DEBUG_OFF) || (level > RV_DEBUG_DEBUG) ||
        (level > g_rv_debug_level) || (fmt == NULL)) {
        return;
    }
    ...
    if (g_rv_debug_sink != NULL) {
        g_rv_debug_sink(level, title, function_name, message,
                        g_rv_debug_user_data);
        return;
    }

    rv_debug_default_sink(level, title, function_name, message,
                          g_rv_debug_user_data);
}
