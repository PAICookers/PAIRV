/*
 * CPU-only, real-dimension checks for the operators surrounding PAICore.
 * References and frozen QAT parameters come from generated test_data.h; this
 * executable does not deploy artifacts or exercise the SNN Head chain.
 */
#include <math.h>
#include <stddef.h>
#include <stdint.h>
...
/* Real tensors are static to avoid exhausting the target stack. */
...
static int check_layernorms(void)
{
    ...
    rv_layernorm_f32(SNN_LN1_X, g_ln1_y, 1U, SNN_REAL_LN1_DIM, SNN_LN1_WEIGHT,
                     SNN_LN1_BIAS, RV_LAYERNORM_DEFAULT_EPS);
    float max_d =
        max_abs_diff(g_ln1_y, SNN_LN1_Y_REF, SNN_REAL_LN1_DIM, &finite);
    ...
    rv_layernorm_f32(SNN_LN2_X, g_ln2_y, 1U, SNN_REAL_LN2_DIM, SNN_LN2_WEIGHT,
                     SNN_LN2_BIAS, RV_LAYERNORM_DEFAULT_EPS);
        ...
        if (g_quant_y[i] != SNN_QUANT_REF[i]) {
            ...
            if (mismatch <= 8) {
                printf("  quant mismatch i=%lu got=%d ref=%d\r\n",
                       (unsigned long)i, (int)g_quant_y[i],
                       (int)SNN_QUANT_REF[i]);
            }
        }
    }
...
static int check_fc3_real_size(void)
{
    ...
    rv_fc_s8_s32(g_quant_y, SNN_FC3_WEIGHT, SNN_FC3_BIAS, g_fc3_acc, 1U,
                 SNN_REAL_FC3_IN, SNN_REAL_FC3_OUT);
    ...
    for (size_t i = 0; i < SNN_REAL_FC3_OUT; ++i) {
        if (g_fc3_acc[i] != SNN_FC3_ACC_REF[i]) {
            ...
            printf("  fc3 mismatch o=%lu got=%ld ref=%ld\r\n", (unsigned long)i,
                   (long)g_fc3_acc[i], (long)SNN_FC3_ACC_REF[i]);
        }
    }
...
static int check_dequantize_action(void)
{
    ...
    const float max_d =
        max_abs_diff(g_action, SNN_ACTION_REF, SNN_REAL_FC3_OUT, &finite);
...
int main(void)
{
    ...
    printf("SNN Head CPU ops selftest (N307FD/QEMU)\r\n");
