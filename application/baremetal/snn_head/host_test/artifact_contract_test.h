...
typedef struct snn_head_layer_contract_s {
    ...
    const char *asset_leaf;   /* assets subdirectory name */
...
/* Build the artifact path from the compile-time assets directory. */
static int snn_head_test_asset_path(const snn_head_layer_contract_t *c,
                                    char *buf, size_t buf_size)
{
    const int written = snprintf(buf, buf_size, "%s/%s/compile_artifacts.bin",
                                 SNN_HEAD_ASSET_DIR, c->asset_leaf);
    return ((written > 0) && ((size_t)written < buf_size)) ? 0 : 1;
}

/* Validate timing + mapping metadata against the layer contract. */
static int
snn_head_test_check_contract(const uint8_t *data, size_t size,
                             ...
                             rvrt_artifact_input_mapping_view_t *input_view_out)
{
    ...
    if ((runtime.timesteps != SNN_HEAD_TEST_TIMESTEPS) ||
        (runtime.pipeline_latency != SNN_HEAD_TEST_TICK_DEPTH) ||
        (runtime.completion_sync_timestep != SNN_HEAD_TEST_TIMESTEPS) ||
        (runtime.output_time_encoding != RVRT_OUTPUT_TIME_ENCODING_STREAM) ||
        (input_view.entry_count != c->input_entries) ||
        (input_view.element_count != c->input_entries) ||
        ...
        printf("  runtime: timesteps=%u pipeline_latency=%u "
               "completion_sync_timestep=%u "
               "output_time_encoding=%u\n",
               (unsigned)runtime.timesteps, (unsigned)runtime.pipeline_latency,
               (unsigned)runtime.completion_sync_timestep,
               (unsigned)runtime.output_time_encoding);
        printf("  input:  entries=%u elements=%u bit_width=%u "
               "(want %u/%u/%u)\n",
               (unsigned)input_view.entry_count,
               (unsigned)input_view.element_count,
               (unsigned)input_view.bit_width, (unsigned)c->input_entries,
 ...
 */
static int
snn_head_test_check_encode(const rvrt_artifact_input_mapping_view_t *input_view,
                           const snn_head_layer_contract_t *c)
{
    ...
    for (;;) {
        ...
        const rvrt_codec_status_t st = rvrt_encode_input_chunk(
            input_view, &cursor, input, input_size, frames,
            (uint32_t)(sizeof(frames) / sizeof(frames[0])), &frame_count);
        total_frames += frame_count;
        if (st == RVRT_CODEC_STATUS_DONE) {
            break;
        }
        if (st == RVRT_CODEC_STATUS_BUFFER_FULL) {
            continue;
        }
        printf("%s: encode failed: %s\n", c->name,
               rvrt_codec_status_string(st));
        free(input);
        return 5;
    }
    free(input);
 ...
 *
 * A missing deployed asset is a hard failure.
 */
static int snn_head_run_layer_test(const snn_head_layer_contract_t *c)
{
    char path[512];
    if (snn_head_test_asset_path(c, path, sizeof(path)) != 0) {
        printf("%s: asset path too long\n", c->name);
        return 1;
    }
    ...
    if (rc == 0) {
        printf("%s: contract + encode PASS (input=%u %ubit, output=%u/%u "
               "kind=%u dtype=%u)\n",
               c->name, (unsigned)c->input_entries,
               (unsigned)c->input_bit_width, (unsigned)c->output_entries,
               (unsigned)c->output_elements, (unsigned)c->output_kind,
               (unsigned)c->output_dtype);
    }
    return rc;
