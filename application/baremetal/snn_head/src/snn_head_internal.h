...
/*
 * SNN Head 分层实现的内部共享头：跨文件共享的宏、参数/artifact 符号、artifact
 * 上下文 与契约类型、共享 buffer、公共 helper 及各层入口声明。公共 API 见
 * snn_head.h。 本头只做声明，共享 buffer 的唯一定义在 snn_head.c（此处仅
 * extern）。
 */
...
#define SNN_HEAD_TENSOR_WORKSPACE_BYTES                                        \
    (SNN_HEAD_TIMESTEPS * SNN_HEAD_HIDDEN_DIM * sizeof(float))
/* fc2 int32 膜电位每行 1536*4=6144 字节；int8 输入按此 stride 扩排，让本行
 * int32 输出 原地覆盖同行已消费的 int8 输入而不踩下一行。 */
#define SNN_HEAD_VOLTAGE_STRIDE_BYTES (SNN_HEAD_HIDDEN_DIM * sizeof(int32_t))
/* 层间共享帧 buffer 容量。runner 用它分块编码输入；完成同步的 work frame 直接在
 * IRQ 中解码到输出，不再按最坏 VOLTAGE RX 帧数预留。 */
#define SNN_HEAD_FRAME_BUF_FRAMES 256U
...
/* 逐层中间结果 dump（仅调试；默认关闭，对部署路径零影响）。定义
 * -DSNN_HEAD_ENABLE_DUMP （board_test 用 make
 * DUMP=1）后各层在阶段边界逐元素打印，未定义时展开为空语句。 sink 由
 * board_test/snn_head_dump_sink.c 提供。 */
typedef enum {
...
/* LN 和量化参数（定义在 snn_head_params.c，放 .large_const_data 段，不占
 * DLM）。 */
...
/* fc3 artifact 由 Makefile 从应用 assets 生成。 */
extern const uint8_t snn_head_fc3_artifact_start[];
extern const uint8_t snn_head_fc3_artifact_size[];

/*
 * 三块跨层共享 buffer 的唯一定义在 snn_head.c，此处仅声明（外部链接，单实例、落
 * .bss）。 tensor_workspace  : 主张量 workspace，五层共用，最大
 * float32/int32[8][1536] = 48KB， 底层 uint8_t，各层按阶段解释成
 * float/int8/int32，aligned(4)。 layer_frame_buf   : 层间共享帧
 * buffer，用于输入编码 workspace 和 reset completion。 fc2_voltage_state :
 * VOLTAGE 的 [timestep][element] lane 掩码状态，仅 fc2/fc3
 * 用；部分值直接拼入输出 int32 槽，runner
 * 在一个完整层样本开始时清零，支持一次完成 同步中多个 timestep 的 lane frame
 * 交错到达。
 */
...
extern rvrt_voltage_decode_state_t fc2_voltage_state[SNN_HEAD_TIMESTEPS]
                                                    [SNN_HEAD_HIDDEN_DIM];
...
typedef struct snn_head_layer_artifact_context_s {
    ...
    rvrt_artifact_input_mapping_view_t
        input_view; /**< 输入 tensor 到 work frame 的映射。 */
...
/**
 * @brief 读取单层 PAICore artifact 并借出 runtime 和 I/O mapping。定义在
 * snn_head.c。
 *
 * 只做最小读取，静态契约由 snn_head_validate_layer_artifact() 检查。
 *
 * @param artifact_start artifact 二进制起始地址（通常来自 Flash objcopy
 * 符号）。
...
/** @brief Convert an objcopy absolute size symbol address to artifact bytes. */
size_t snn_head_artifact_size(const uint8_t *artifact_size_symbol);

/**
 * @brief 校验单层 artifact 是否满足固定分层契约。定义在 snn_head.c。
 *
 * 只判断 runtime 和 I/O mapping 的静态形状与语义，不访问 config frame
 * 或权重内容。
 *
...
/**
 * @brief 执行 fc2 层（LN2 -> quant -> fc2 线性 -> li_out 膜电位）。定义在
 * snn_head_fc2.c。
 * @return 成功返回 true；完成后 tensor_workspace 开头为
 * float32[8][1536]（li_out 膜电位反量化）。
 */
bool snn_head_run_fc2(void);

/**
 * @brief 执行 fc3 层（quant -> fc3 线性 -> acc_int32 -> 反量化到
 * action）。定义在 snn_head_fc3.c。
...
/**
 * @brief Deploy and execute one complete 8-timestep PAICore layer sample.
 *
 * Input/output capacities and strides are byte counts. Timing-enabled builds
 * record deploy and sample details in the currently active layer profile.
 *
